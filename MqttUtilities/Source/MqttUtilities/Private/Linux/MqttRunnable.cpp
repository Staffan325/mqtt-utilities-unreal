...
FMqttRunnable::FMqttRunnable(UMqttClient *mqttClient) : FRunnable(), TaskQueue(new std::queue<FMqttTaskPtr>()), TaskQueueLock(new FCriticalSection()), client(mqttClient)
{
}
...
uint32 FMqttRunnable::Run()
{
	...
	if (!CaFile.empty() || !CaPath.empty())
	{
		...
		int tlsStatus = connection.tls_set(
				...
				NULL);
		connection.tls_insecure_set(AllowInsecure);
